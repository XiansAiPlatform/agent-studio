...
/**
 * DELETE /api/metrics/agents/{agentName}/activation/{activationId}
 * Deletes all performance-metric records for an agent activation.
  ...
  const handler = withParticipantAdmin(
    async (req: NextRequest, { tenantContext }: ApiContext) => {
      try {
        ...
        const backendPath = `/api/v1/admin/tenants/${encodeURIComponent(tenantId)}/metrics/agents/${encodeAgentNamePath(agentName)}/activation/${encodeAgentNamePath(activationId)}`
        ...
        return handleApiError(error, 'metrics DELETE by activation', {
          fallbackMessage: 'Failed to delete performance metrics',
        })
      }
    }
  )

 ...
 * Deletes all messages (across every participant) for an agent activation.
  ...
  const handler = withParticipantAdmin(
    async (req: NextRequest, { tenantContext }: ApiContext) => {
      try {
        ...
        const backendPath = `/api/v1/admin/tenants/${tenantId}/messaging/agents/${encodeURIComponent(agentName)}/activation/${encodeURIComponent(activationId)}`
        ...
        return handleApiError(error, 'messaging DELETE by activation', {

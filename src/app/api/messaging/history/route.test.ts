...
function historyUrl(extra: Record<string, string> = {}) {
  const params = new URLSearchParams({
    agentName: 'Support',
    activationName: 'prod',
    ...extra,
  })
  return `http://localhost/api/messaging/history?${params.toString()}`
}
...
describe('GET /api/messaging/history view-as wiring', () => {
  beforeEach(() => {
  ...
  it('uses the session email when viewAsParticipantId is absent', async () => {
    ...
    const res = await GET(new NextRequest(historyUrl()))
  ...
  it('honors viewAsParticipantId for a system admin', async () => {
    ...
    const res = await GET(
      new NextRequest(
        historyUrl({ [VIEW_AS_PARTICIPANT_QUERY_PARAM]: 'target@example.com' })
      )
    )

    expect(res.status).toBe(200)
    const historyCall = get.mock.calls.find((call) =>
      String(call[0]).includes('/messaging/history')
    )
    expect(String(historyCall?.[0])).toContain('participantId=target%40example.com')
  })

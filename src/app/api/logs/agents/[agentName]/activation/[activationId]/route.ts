  ...
  const handler = withParticipantAdmin(
    async (req: NextRequest, { tenantContext }: ApiContext) => {
      try {
        ...
        const backendPath = `/api/v1/admin/tenants/${encodeURIComponent(tenantId)}/logs/agents/${encodeAgentNamePath(agentName)}/activation/${encodeAgentNamePath(activationId)}`

...
export function XiansMcpPanel({ agentName, activationName }: { agentName: string; activationName: string }) {
  ...
  return (
    <section className="mb-6 space-y-3 rounded-xl border bg-card p-4 sm:p-6" aria-label="Xians MCP">
      ...
      <p className="text-sm text-muted-foreground">Connect an MCP client to the Xians Platform.</p>
      {configuration ? (
      ...
      <p className="text-xs text-muted-foreground">
        Authentication: Xians admin API key (Bearer token). Generate one in{' '}
        <Link href="/developer/secrets" className="underline underline-offset-2">Developer → Secrets → Admin API Keys</Link>.
      </p>
    </section>
  )
}

...
export function AgentCard({ agent, isNewlyCreated, currentUserEmail, onClick }: AgentCardProps) {
  ...
  return (
    <Card 
      className={`group transition-all duration-200 cursor-pointer border shadow-none overflow-hidden ${
        isInactive 
        ...
        <div className="space-y-1.5">
          <div className="flex flex-wrap items-center gap-2">
            ...
            {isNewlyCreated && !isInactive && (
              <Badge 
                variant="default" 
                className="text-xs shrink-0 bg-emerald-600 hover:bg-emerald-600"
              >
                NEW
              </Badge>
            )}
